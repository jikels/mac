...
def train_meta(
    ...
    '''
    Train a model on a set of tasks using the FAMLE algorithm.
    ...
    for meta_count in range(meta_iter):
        ...
        # create task array for famle model
        if model.cuda_enabled:
            tasks_tensor = torch.LongTensor([[task_index] for _ in range(minibatch)]).cuda()
        else:
            tasks_tensor = torch.LongTensor([[task_index] for _ in range(minibatch)])
